...
function displayPairingUI(pairingCode, serverUrl, aiTool, aiToolVersion, projectName, computerName) {
  ...
  // Display QR code without inner frame (cleaner look)
  qrcode.generate(qrData, { small: true }, (code) => {
    code.split('\n').forEach(line => {
      if (line.trim()) {
        // Center QR code line within the main box
        const paddingLeft = '  ';
        const paddingRight = '  ';
        console.log(chalk.bold.cyan('│') + paddingLeft + chalk.white(line) + paddingRight + chalk.bold.cyan('│'));
      }
    });
  });
  console.log(chalk.bold.cyan('│                                          │'));
