...
const { getApiUrl, isLocal } = require('../config/environment');
...
async function checkVersion() {
  ...
  // For local environment, append -local to package name for testing
  const packageName = isLocal() ? `${packageInfo.name}-local` : packageInfo.name;

  logger.debug(`Checking version: ${packageName}@${packageInfo.version}`);
