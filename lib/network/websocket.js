...
class WebSocketManager {
  constructor(serverUrl, sessionId, buffer) {
    ...
    this.HEARTBEAT_TIMEOUT = 13000; // 13 seconds without any message = network loss (server pings every ~5s)
  ...
  async handleCatchupRequest(message) {
    // Only show on console in debug mode, but always log to file
    logger.debugInfo(`Catchup requested from seq ${message.lastSeq}`);
    if (process.env.DEBUG === '1' || process.argv.includes('--debug')) {
  ...
  async reconnect() {
    ...
    if (success) {
      ...
      // Show error message
      const msg = '❌ Failed to reconnect after max attempts';
      process.stderr.write('\x1b[2K\r' + chalk.red(msg) + '\n');
      console.log('');
      console.log(chalk.red('Please restart the session:'));
      console.log(chalk.cyan('  termly start'));
      process.exit(1);
    }
  }
  ...
  resetHeartbeatTimer(isInitial = false) {
    ...
    const timeout = isInitial ? this.HEARTBEAT_TIMEOUT * 2 : this.HEARTBEAT_TIMEOUT;
