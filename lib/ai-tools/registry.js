...
// On Windows, returns full path to command for .cmd/.bat files
...
async function commandExists(command) {
  try {
    ...
    // On Windows, return first line (full path) if it's a .cmd or .bat file
    if (isWindows) {
      const firstLine = stdout.trim().split('\n')[0].trim();
      // Only return path for .cmd/.bat files - these need full paths on Windows
      if (firstLine.toLowerCase().endsWith('.cmd') || firstLine.toLowerCase().endsWith('.bat')) {
        return firstLine;
      }
    }
