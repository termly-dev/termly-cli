...
function processInput(input) {
  ...
  // Handle internal system command for mobile connection
  if (trimmed === '__MOBILE_CONNECTED__') {
    return {
      text: '', // Will be handled by redrawScreen
      shouldExit: false,
      shouldRedraw: true
    };
  }
...
/**
 * Redraw screen with animation and welcome
 */
async function redrawScreen() {
  ...
  // Show animated logo
  await displayAnimatedLogo(4000, 150);
  ...
  process.stdin.on('data', (chunk) => {
    ...
    if (buffer.includes('\n')) {
      ...
      lines.forEach(async (line) => {
        ...
        // Redraw screen if requested (mobile connected)
