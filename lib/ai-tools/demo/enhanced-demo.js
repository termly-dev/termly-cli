...
class ScreenRenderer {
  ...
  renderFooter() {
    ...
    // Clear line 1 completely
    this.moveCursor(footerRow, 1);
    process.stdout.write('\x1B[2K'); // Clear entire line
    process.stdout.write(' '.repeat(termWidth)); // Fill with spaces
    this.moveCursor(footerRow, 1);
    process.stdout.write('\x1B[2K'); // Clear again
...
class DemoController {
  ...
  setupResize() {
    ...
    this.resizeHandler = () => {
      ...
      // Clear footer lines using OLD width first to remove any artifacts
      const footerRow = HEADER_HEIGHT + CHAT_HEIGHT + 3;
      for (let i = 0; i < FOOTER_HEIGHT; i++) {
        this.renderer.moveCursor(footerRow + i, 1);
        process.stdout.write('\x1B[2K'); // Clear entire line
        process.stdout.write(' '.repeat(Math.max(oldWidth, newWidth))); // Fill with spaces
        this.renderer.moveCursor(footerRow + i, 1);
        process.stdout.write('\x1B[2K'); // Clear again
      }
