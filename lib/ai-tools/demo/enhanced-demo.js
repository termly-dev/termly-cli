...
class ScreenRenderer {
  ...
  addUserInput(text) {
    this.chatBuffer.addLine(chalk.cyan(text));
    this.currentInput = '';
  }

  addAIResponse(text) {
    this.chatBuffer.addLine(chalk.hex(COLOR_SECONDARY)(text));
  }
}
...
class DemoController {
  ...
  handleInput(input) {
    // Filter out system messages (mobile connected/disconnected)
    if (input.startsWith('__MOBILE_') ||
        input.includes('connected') && input.includes('Mobile') ||
        input.includes('disconnected') && input.includes('Mobile')) {
      // Ignore system messages, don't display in chat
      return;
    }
    ...
    if (inputIdx >= 0) {
      lines[inputIdx] = chalk.cyan(input);
    }
