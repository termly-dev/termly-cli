...
// Original logo frames from logo.js (7 lines each)
...
class ScreenRenderer {
  ...
  renderHeader() {
    ...
    // Render logo only (centered)
    for (let i = 0; i < HEADER_HEIGHT; i++) {
      const logoLine = logo[i] || '';
      const colored = chalk.hex(COLOR_PRIMARY)(logoLine);
  ...
  renderChat() {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (line === '__INPUT__') {
        // Render current input with cursor (no prefix)
  ...
  renderFooter() {
    ...
    if (promptOnTop) {
      ...
      // Render banner1 (only if visible on screen)
      if (banner1Pos >= 0 && banner1Pos < termWidth) {
        this.moveCursor(footerRow, banner1Pos + 1);
        const visibleText = banner1Text.substring(0, termWidth - banner1Pos);
        process.stdout.write(chalk.hex(COLOR_PRIMARY)(visibleText));
      }

      // Render prompt on top (only if visible on screen)
      if (promptPos >= 0 && promptPos < termWidth) {
        this.moveCursor(footerRow, promptPos + 1);
        const visibleText = promptText.substring(0, termWidth - promptPos);
      ...
      // Render line 1: banner1 (only if visible on screen)
      this.moveCursor(footerRow, 1);
      if (banner1Pos >= 0 && banner1Pos < termWidth) {
        this.moveCursor(footerRow, banner1Pos + 1);
        const visibleText = banner1Text.substring(0, termWidth - banner1Pos);
      ...
      // Render line 2: prompt (only if visible on screen)
      this.moveCursor(footerRow + 1, 1);
      if (promptPos >= 0 && promptPos < termWidth) {
...
class DemoController {
  ...
  setupResize() {
    ...
    this.resizeHandler = () => {
      ...
      this.renderer.animState.terminalWidth = process.stdout.columns || 80;
