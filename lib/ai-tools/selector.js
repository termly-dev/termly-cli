...
async function selectManualTool(toolName) {
  ...
  if (!tool) {
    ...
    console.error('  • Claude Code (claude-code or "claude code")');
    console.error('  • Aider (aider)');
    console.error('  • GitHub Copilot (github-copilot)');
    console.error('  • Cursor (cursor)');
    console.error('  • Cody (cody)');
    console.error('');
...
async function autoSelectTool() {
  ...
  if (installedTools.length === 0) {
    ...
    console.error('  • Cursor: https://cursor.sh');
    console.error('');
