...
class PTYManager {
  constructor(tool, workingDir, buffer) {
    ...
    this.initialRows = null; // Store initial rows for Windows (locked height)
  ...
  start(additionalArgs = []) {
    ...
    try {
      ...
      this.initialRows = initialRows; // Store for Windows resize lock
  ...
  resize(cols, rows) {
    if (this.ptyProcess) {
      ...
      // On Windows, only resize width (cols) to avoid ConPTY flickering
      // ConPTY redraws entire buffer on height change, causing visual issues
      if (isWindows) {
        const keepRows = this.initialRows || 24;
        this.ptyProcess.resize(cols, keepRows);
        logger.debug(`PTY resized to ${cols}x${keepRows} (Windows: height locked)`);
      } else {
        // On Unix, resize both dimensions normally
