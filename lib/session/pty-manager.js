  ...
  start(additionalArgs = []) {
    ...
    try {
      ...
      if (isWindows) {
        ...
        // Full paths need quotes, but must be in a single string passed to /c
        const isFullPath = this.tool.command.includes('\\') || this.tool.command.includes('/');
        const quotedCommand = isFullPath ? `"${this.tool.command}"` : this.tool.command;

        // Combine command with args into single string for /c
        const fullCommand = args.length > 0
          ? `${quotedCommand} ${args.join(' ')}`
          : quotedCommand;
