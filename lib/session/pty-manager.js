  ...
  handlePTYOutput(data) {
    ...
    if (filtered.length > 0) {
      ...
      if (os.platform() === 'win32') {
        ...
        // Log timing for every output to see patterns
        if (process.env.DEBUG === '1' || process.argv.includes('--debug')) {
          logger.debug(`Dedup check: ${timeSinceLastOutput}ms since last, same data: ${this.lastOutputData === filtered}`);
        }

        if (this.lastOutputData === filtered && timeSinceLastOutput < this.duplicateThresholdMs) {
          // This is a duplicate - skip it
          if (process.env.DEBUG === '1' || process.argv.includes('--debug')) {
            logger.debug(`>>> DUPLICATE SKIPPED (${timeSinceLastOutput}ms)`);
          }
          return;
        }

        // Update deduplication state
        this.lastOutputData = filtered;
