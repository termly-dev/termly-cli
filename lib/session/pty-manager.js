...
class PTYManager {
  ...
  handlePTYOutput(data) {
    ...
    // Windows workaround: normalize screen clear escape sequences
    ...
    if (os.platform() === 'win32' && filtered.startsWith('\x1b[H\x1b[K')) {
      filtered = '\x1b[2J\x1b[3J\x1b[H' + filtered.slice(6);
      if (process.env.DEBUG === '1' || process.argv.includes('--debug')) {
        logger.debug('Normalized Windows screen clear to Mac format (\\x1b[2J\\x1b[3J\\x1b[H)');
      }
    }
      ...
      if (os.platform() === 'win32') {
        ...
        if (duplicate) {
          ...
          if (process.env.DEBUG === '1' || process.argv.includes('--debug')) {
            ...
            logger.debug(`>>> DUPLICATE SKIPPED (${timeSinceDuplicate}ms)`);
          }
          return;
        }
