...
class PTYManager {
  ...
  handlePTYOutput(data) {
    ...
    if (filtered.length > 0) {
      // Debug: Log raw data to see cursor sequences
      const escapedData = filtered.replace(/\x1b/g, '\\e');
      if (escapedData.includes('\\e')) {
        logger.debug(`PTY output with escapes: ${escapedData.substring(0, 200)}`);
      }
