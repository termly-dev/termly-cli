  ...
  start(additionalArgs = []) {
    ...
    try {
      ...
      if (isWindows) {
        ...
        // On Windows, always wrap full paths in quotes for cmd.exe
        // Even paths without spaces need quotes when they're full paths
        const isFullPath = this.tool.command.includes('\\') || this.tool.command.includes('/');
        const command = isFullPath ? `"${this.tool.command}"` : this.tool.command;
